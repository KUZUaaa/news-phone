<template>
    <div class="article-container">
      ...
      >
        <van-button slot="left" to="/audit" class="backBtn">
  ...
  import {getArticleById,articlePass,articleRefuse} from '@/api/article'
  ...
  export default {
    ...
    methods: {
        ...
        async onPass(){
          try{
            await articlePass(this.articleId)
            this.$router.back()
          }catch{
            this.$toast("通过失败，请稍后再试")
          }
        },
        async onRefuse(){
          try{
            await articleRefuse(this.articleId)
            this.$router.back()
          }catch{
            this.$toast("审核失败，请稍后再试")
          }
        },
    }
  }
